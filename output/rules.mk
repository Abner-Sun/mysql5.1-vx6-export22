...
#########################################################################################
wind_usr := C:/vxworks/GPPVE3.8_X86/vxworks-6.8/target/usr
wind_base := C:/vxworks/GPPVE3.8_X86/vxworks-6.8
wind_components := C:/vxworks/GPPVE3.8_X86/components
...
#endif
OUTPUT := $(shell cd $(OUTPUT) && pwd)

PREFIX := $(OUTPUT)
PREFIX := $(shell cd $(PREFIX) && pwd)
...
CC := C:/vxworks/GPPVE3.8_X86/gnu/4.1.2-vxworks-6.8/x86-win32/bin/ccpentium
CCP := C:/vxworks/GPPVE3.8_X86/gnu/4.1.2-vxworks-6.8/x86-win32/bin/c++pentium
LD := C:/vxworks/GPPVE3.8_X86/gnu/4.1.2-vxworks-6.8/x86-win32/bin/ldpentium
AR := C:/vxworks/GPPVE3.8_X86/gnu/4.1.2-vxworks-6.8/x86-win32/bin/arpentium
RANLIB := C:/vxworks/GPPVE3.8_X86/gnu/4.1.2-vxworks-6.8/x86-win32/bin/ranlibpentium
STRIP := $(CROSS)strip
...
# sfh add CFLAGS += -DNDEBUG -O3 -fomit-frame-pointer
endif
...
$(EXENAMEP): $(CONFIG_H_FILE) $(EXEDEPS) $(EXEOBJS)
	$(CCP) -Wl -o $(EXENAMEP) $(EXEOBJS) $(LDFLAGS) $(EXELDFLAGS) $(EXTRALDFLAGS)
	mv $(EXENAMEP) $(TOP)/bin/
	@echo "========= Link execute $@ end ========"
endif
...
#因为在规则中$<表示第一个依赖文件名
%.d: %.c $(CONFIG_H_FILE)
	@$(CC) -MM $(CFLAGS) $(EXECFLAGS) $(LIBCFLAGS) $(EXTRACFLAGS) $< > $@
...
%.dd: %.cpp $(CONFIG_H_FILE)
	@$(CCP) -MM $(CPPFLAGS) $(CFLAGS) $(EXECFLAGS) $(LIBCFLAGS) $(EXTRACFLAGS) $< > $@
